    ...
    return (
        <AdminLayout>
            <div className="mb-6 flex items-center gap-4">
                <Link to="/dashboard" className="p-2 bg-white rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-600 transition-colors"><ArrowLeft size={20} /></Link>
                <div className="text-sm font-medium text-gray-400">Beranda / Detail Quiz</div>
            </div>

            <div className="bg-[#1b6d39] rounded-2xl p-8 text-white mb-8 flex flex-col md:flex-row justify-between items-start md:items-center relative overflow-hidden shadow-sm gap-6">
                <div className="relative z-10">
                    <h1 className="text-3xl font-bold mb-2">{quiz.title}</h1>
                    <p className="text-green-100 font-medium">{quiz.subject?.name} • {quiz.student_class?.name}</p>
                </div>
                
                <div className="relative z-10 flex items-center gap-5 w-full md:w-auto justify-between md:justify-end border-t border-green-700/50 pt-4 md:border-0 md:pt-0">
                    <button 
                        onClick={() => navigate('/dibagikan', { state: { quiz: quiz } })}
                        className="bg-[#F2994A] hover:bg-[#e0893d] transition-colors text-white px-6 py-3.5 rounded-xl font-bold flex items-center gap-2 shadow-md shrink-0"
                    >
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 text-gray-500 text-sm border-b border-gray-100">
                                ...
                                <th className="p-4 font-bold text-center">Waktu</th>
                        ...
                        <tbody>
                            {quiz.student_results?.map((result, idx) => (
                                <React.Fragment key={idx}>
                                    <tr onClick={() => toggleRow(idx)} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors cursor-pointer group">
                                        <td className="p-4 font-bold text-[#1b6d39] pl-6 flex items-center gap-2">
                                        ...
                                        <td className="p-4 text-right pr-6">
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg font-black text-sm shadow-sm ${result.score >= 70 ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                                        ...
                                        <tr className="bg-gray-50/50 border-b border-gray-100">
                                            <td colSpan="4" className="p-6">
                                                <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                                                    <h4 className="font-bold text-gray-800 mb-3 text-sm uppercase tracking-wider">Analisa Pemahaman: {result.student_name}</h4>
                                                        ...
                                                        {JSON.parse(result.answers_data).map((ans, i) => (
                                                            <div key={i} className={`p-3 rounded-lg border flex gap-3 ${ans.is_correct ? 'bg-green-50/50 border-green-100' : 'bg-red-50/50 border-red-100'}`}>
                                                                ...
                                                                <div className="flex-1">
                                                                    <p className="font-semibold text-gray-900 text-sm mb-1">{ans.question_text}</p>
                            ...
                            {(!quiz.student_results || quiz.student_results.length === 0) && (
                                <tr>
                                    <td colSpan="4" className="p-8 text-center text-gray-400 font-medium">Belum ada siswa yang mengerjakan kuis ini.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
